<template>
  <el-table-column v-bind="config">
    ...
    <template v-if="config.headerSlotName" #header="scope">
      <div v-if="config.headerSlotName === 'tooltip'" class="c-flex-ycenter">
        ...
        <el-tooltip
          class="box-item"
          effect="dark"
          :content="config.headerSlotParams.content"
          placement="top"
        >
      ...
      <slot
        v-else
        :name="config.headerSlotName"
        v-bind="config.headerSlotAttr"
        v-on="config.headerSlotOn"
      ></slot>
    </template>
    <!-- 自定义列模板 -->
    <template v-if="!config.child && config.slotName" #default="scope">
      ...
      <span
        v-else-if="config.slotName === 'addArrow'"
...
function formatValue(type: string, params: unknown) {
  ...
  switch (type) {
    case 'text':
      value = (params as Record<string, any>).value
      break
    case 'number':
    case 'price':
